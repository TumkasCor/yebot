...
class TestBot extends TelegramBot{
	 ...
	 */
	function cmd_call(){
		$this->api->sendMessage( "+78006001410, @" . $this->result["message"]["from"]["username"] . "." );
	}
	...
	function cmd_default(){
		...
		if( stripos( $this->result["message"]["text"], "кц" ) !== false ){
			$this->api->sendMessage( "+78006001410" );
		}
		// Если пользователь хочет поддержки
		elseif( stripos( $this->result["message"]["text"], "номер" ) !== false ){
			$this->api->sendMessage( "+78006001410" );
		}
